...
import fx                                                        # noqa: E402
from picture_plan_v9 import (CLIN, CLIPS, DOCS, FALLBACK,  # noqa: E402
                             FLASH, PARALLAX, PLAN, STILLS)

W, H, FPS = fx.W, fx.H, fx.FPS
AUDIO = ROOT / "final_video/mrbeast_audio_v6/MRBEAST_V6_STORY_MASTER.wav"
SUFFIX = os.environ.get("BUILD_SUFFIX", "V9")
...
def _first_usable(group: str, dur: float):
    for it in BROLL.get(group) or []:
        p = ROOT / it["file"]
...
def take_broll(group: str, dur: float, prog_t: float) -> dict:
    """The segment's own footage first, then a declared neighbour, then any.

    V8 raised here and the build stopped. That was right when a group meant a
    subject and running dry meant the subject was unavailable. In V9 a group
    means one sentence, so running dry only means that sentence found fewer
    clips than it needed - and the honest fallback is a neighbouring sentence,
    recorded so the drop shows up in the summary rather than passing silently.
    """
    it = _first_usable(group, dur)
    if it is not None:
        return it
    for alt in FALLBACK.get(group, []):
        it = _first_usable(alt, dur)
        if it is not None:
            DEGRADED[group] = DEGRADED.get(group, 0) + 1
            return it
    for alt in sorted(BROLL, key=lambda g: -len(BROLL.get(g) or [])):
        if alt == group:
            continue
        it = _first_usable(alt, dur)
        if it is not None:
            ...
            return it
    raise Exhausted(
        f"no unspent clip anywhere is long enough at {prog_t:.2f}s "
        f"({dur:.2f}s needed; group '{group}'). Fetch more - "
        f"looping is banned.")
...
def render_flash(key: str, dur: float, dest: Path) -> None:
    ...
    each = dur / len(imgs)
    # Frames per image must SUM to the shot's frame count. Rounding each part
    # independently loses the remainder: 5 images over 1.2s gave 5 x round(7.2)
    # = 35 frames where 36 were needed, and exact_frames refuses to pad with a
    # freeze, so the whole build died on the last shot it rendered.
    want_total = max(len(imgs) * 2, int(round(dur * FPS)))
    base, extra = divmod(want_total, len(imgs))
    counts = [base + (1 if i < extra else 0) for i in range(len(imgs))]
    parts = []
...
def main() -> int:
    ...
    if DEGRADED:
        print("\n[degraded] segments that ran out of their own footage and "
              "borrowed from a neighbour:")
        for g, n in sorted(DEGRADED.items(), key=lambda kv: -kv[1]):
            print(f"    {g:>6}  {n} shot(s)")
        print("    fetch more for these before this is a final cut")
    print(f"[OK] {OUT}")
